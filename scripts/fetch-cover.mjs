...
//   node scripts/fetch-cover.mjs movie  "Inception" [year]
...
export const COVERS_DIR = path.join(__dirname, '..', 'src', 'content', 'media', 'covers');
...
export async function fetchAlbum(artist, album) {
  ...
  return { url: artworkUrl, slug: toSlug(artist, album), ext, year: parseInt(match.releaseDate) || null };
}

export async function fetchBook(title, author) {
  ...
  return { url: `https://covers.openlibrary.org/b/id/${match.cover_i}-L.jpg`, slug: toSlug(title), ext: 'jpg', year: null };
}

export async function fetchMovie(title, year) {
  const key = '413a72cbcdde5dfa023fb545586e076f';
  ...
  const creditsRes = await fetch(`https://api.themoviedb.org/3/movie/${match.id}/credits?api_key=${key}`);
  ...
  return {
    ...
    slug: toSlug(title),
    ext: 'jpg',
    year: parseInt(match.release_date) || null,
    director,
  };
}

export async function fetchGame(title) {
  const query = encodeURIComponent(title);
  ...
  return { url, slug: toSlug(title), ext: 'jpg', year: null };
}
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  node scripts/fetch-cover.mjs movie  "Title"   [year]
  ...
  try { await downloadTo(result.url, dest); }
  catch (err) { console.error(`Failed to download: ${err.message}`); process.exit(1); }
